...
def _rgb_to_hsv(rgb: torch.Tensor) -> torch.Tensor:
    ...
    v = maxc
    s = delta / (maxc + 1e-7)
    rc = (maxc - r) / (delta + 1e-7)
    gc = (maxc - g) / (delta + 1e-7)
    bc = (maxc - b) / (delta + 1e-7)
    h = torch.zeros_like(v)
    h = torch.where((maxc == r) & (delta > 0), bc - gc, h)
    h = torch.where((maxc == g) & (delta > 0), 2.0 + (rc - bc), h)
    h = torch.where((maxc == b) & (delta > 0), 4.0 + (gc - rc), h)
    h = (h / 6.0) % 1.0
...
class ColorGuide:
    ...
    @classmethod
    def set(cls, rgb: torch.Tensor | None) -> None:
        cls._rgb = rgb
...
class ColorStem(Conv):
    ...
    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 2):
        ...
        self.hsv_smooth = nn.Conv2d(3, 3, 3, 1, 1, groups=3, bias=False)
        _init_depthwise_avg(self.hsv_smooth)
        # 与 RGB 分支相同 stride，保证空间尺寸一致
        self.hsv_enhance = Conv(3, c2, k, s, act=False)
        nn.init.zeros_(self.hsv_enhance.conv.weight)
        if self.hsv_enhance.conv.bias is not None:
            nn.init.zeros_(self.hsv_enhance.conv.bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        feat = super().forward(x)
        hsv = _rgb_to_hsv(x.clamp(0, 1))
        hsv_contrast = hsv - self.hsv_smooth(hsv)
        return feat + self.hsv_enhance(hsv_contrast)
...
class P3ColorEnhance(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int = 8):
        ...
        self.spatial_3 = nn.Sequential(
            nn.Conv2d(fusion_c, hidden, 3, 1, 1, bias=False),
            nn.BatchNorm2d(hidden),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, channels, 3, 1, 1, bias=False),
        )
        self.spatial_5 = nn.Sequential(
            nn.Conv2d(fusion_c, hidden, 5, 1, 2, bias=False),
            nn.BatchNorm2d(hidden),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, channels, 5, 1, 2, bias=False),
        )
        self.scale = nn.Parameter(torch.zeros(1))
        nn.init.zeros_(self.spatial_3[-1].weight)
        nn.init.zeros_(self.spatial_5[-1].weight)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        rgb = ColorGuide.get()
        if rgb is None:
            return x
        rgb_ds = F.interpolate(rgb, size=x.shape[2:], mode="bilinear", align_corners=False)
        hsv = _rgb_to_hsv(rgb_ds.clamp(0, 1))
        hsv_contrast = hsv - self.hsv_smooth(hsv)
        fusion = torch.cat([x, hsv_contrast], dim=1)
        delta = self.spatial_3(fusion) + self.spatial_5(fusion)
...
class ColorPriorBlock(nn.Module):
    ...
    - 3×3 局部对比保留空间位置，利于后续小目标检测头使用
    - 仅插入 backbone 最浅处（距原始 RGB 最近）
    """
    ...
    def __init__(self, channels: int):
        ...
        self.chroma_smooth = nn.Conv2d(3, 3, 3, 1, 1, groups=3, bias=False)
        _init_depthwise_avg(self.chroma_smooth)
        self.enhance = Conv(3, channels, 1, act=False)
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(3, 1, 3, 1, 1, bias=False),
            nn.Sigmoid(),
        )
        nn.init.zeros_(self.enhance.conv.weight)
        if self.enhance.conv.bias is not None:
            nn.init.zeros_(self.enhance.conv.bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        chroma = self.chroma(x)
        local = self.chroma_smooth(chroma)
        contrast = chroma - local
        gate = self.spatial_gate(contrast.abs())
        return x + self.enhance(contrast) * gate
...
class ChromaticContrastGate(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int = 16):
        ...
        self.local_smooth = nn.Conv2d(channels, channels, 3, 1, 1, groups=channels, bias=False)
        _init_depthwise_avg(self.local_smooth)

        self.chroma_proj = nn.Conv2d(channels, 3, 1, bias=False)
        self.chroma_smooth = nn.Conv2d(3, 3, 3, 1, 1, groups=3, bias=False)
        _init_depthwise_avg(self.chroma_smooth)
        self.chroma_enhance = nn.Conv2d(3, channels, 1, bias=False)
        nn.init.zeros_(self.chroma_enhance.weight)
        ...
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(channels * 2 + 3, hidden, 1, bias=False),
            nn.BatchNorm2d(hidden),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, 1, 3, 1, 1, bias=False),
            nn.Sigmoid(),
        )
    ...
    def _rgb_hsv_contrast(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rgb_ds = F.interpolate(rgb, size=x.shape[2:], mode="bilinear", align_corners=False)
        hsv = _rgb_to_hsv(rgb_ds.clamp(0, 1))
        return (hsv - self.hsv_smooth(hsv)) * self.rgb_scale

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        local_mean = self.local_smooth(x)
        local_contrast = x - local_mean

        chroma = self.chroma_proj(x)
        chroma_contrast = chroma - self.chroma_smooth(chroma)

        rgb_contrast = self._rgb_hsv_contrast(x)
        spatial_in = torch.cat([x, local_contrast, rgb_contrast], dim=1)
        s_w = self.spatial_gate(spatial_in)

        c_mean = local_contrast.mean(dim=(2, 3), keepdim=True)
        c_var = local_contrast.var(dim=(2, 3), unbiased=False, keepdim=True)
        c_w = self.channel_gate(torch.cat([c_mean, c_var], dim=1))

        chroma_boost = self.chroma_enhance(chroma_contrast)
        return x + (x * c_w + chroma_boost) * s_w

    # 兼容旧接口
    set_rgb_guide = ColorGuide.set
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            e (float): Expansion ratio.
            use_color_gate (bool): 是否在 Bottleneck 输出后接入 ChromaticContrastGate。
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        if use_color_gate:
            self.color_gate = ChromaticContrastGate(c2)
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply bottleneck with optional shortcut connection."""
        y = self.cv2(self.cv1(x))
        if hasattr(self, "color_gate"):
            y = self.color_gate(y)
...
class C3k2(C2f):
    ...
    def __init__(
        ...
        Args:
            ...
            e (float): Expansion ratio.
            use_color_gate (bool): 是否为该 C3k2 内的 Bottleneck 启用 ChromaticContrastGate。
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                Bottleneck(self.c, self.c, shortcut, g, use_color_gate=use_color_gate),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else C3k(self.c, self.c, 2, shortcut, g)
            if c3k
            else Bottleneck(self.c, self.c, shortcut, g, use_color_gate=use_color_gate)
            for _ in range(n)
        )
