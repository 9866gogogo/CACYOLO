...
class _ClassAwareChromaBranch(nn.Module):
    """类感知色度支路 (CAC-Branch)。.

    共享 HSV 局部对比编码，每类独立零初始化 logit 头 + 可学习类权重：
    - healthy / whitehead 可学习不同的色度响应强度
    - 训练初期各类 scale=0 → 整体等价标准 Detect
    """

    def __init__(self, nc: int, hidden: int = 32):
        super().__init__()
        self.nc = nc
        self.hsv_smooth = nn.Conv2d(3, 3, 3, 1, 1, groups=3, bias=False)
        with torch.no_grad():
            self.hsv_smooth.weight.fill_(1.0 / 9.0)
        # 白穗 S 偏低、V 偏高，提高 S/V 通道权重
        self.register_buffer("sv_weight", torch.tensor([0.5, 1.5, 1.5]).view(1, 3, 1, 1))
        # 共享色度特征编码
        ...
        # 每类独立 logit 头（零初始化）
        self.cls_heads = nn.ModuleList(Conv(hidden, 1, 1, act=False) for _ in range(nc))
        for head in self.cls_heads:
            nn.init.zeros_(head.conv.weight)
            if head.conv.bias is not None:
                nn.init.zeros_(head.conv.bias)
        # 每类可学习权重，零初始化
        self.cls_scale = nn.Parameter(torch.zeros(nc))

    def forward(self, size: tuple[int, int]) -> torch.Tensor | None:
        rgb = ColorGuide.get()
        if rgb is None:
            return None
        rgb_ds = F.interpolate(rgb, size=size, mode="bilinear", align_corners=False)
        hsv = _rgb_to_hsv(rgb_ds.clamp(0, 1))
        contrast = (hsv - self.hsv_smooth(hsv)) * self.sv_weight
        shared = self.shared_enc(contrast)
        scale = self.cls_scale.view(1, self.nc, 1, 1)
        deltas = [head(shared) for head in self.cls_heads]
        return torch.cat(deltas, dim=1) * scale


class ChromaDetect(Detect):
    """色度增强检测头 (CA-Head) + 类感知色度支路 (CAC-Branch)。.

    设计要点（面向 healthy / whitehead 二分类 + 单类预训练微调）：
    - backbone 与 neck 保持 YOLO12 原样 → 预训练权重完整对齐
    - box 回归分支 (cv2) 不变 → 保留 v12s 定位能力
    - P3 分类 logit 叠加类感知 HSV 色度残差 → whitehead 可学习更强色度响应
    - 各类独立零初始化 → 训练起点与 baseline 等价
    """

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end, ch)
        self.chroma_p3 = _ClassAwareChromaBranch(nc)

    def forward_head(
        self, x: list[torch.Tensor], box_head: torch.nn.Module = None, cls_head: torch.nn.Module = None
    ) -> dict[str, torch.Tensor]:
        """在标准 cls 输出上叠加 P3 类感知色度残差。."""
        ...
        for i in range(self.nl):
            cls_out = cls_head[i](x[i])
            if i == 0:
                delta = self.chroma_p3(x[i].shape[2:])
                if delta is not None:
                    cls_out = cls_out + delta
            scores_parts.append(cls_out.view(bs, self.nc, -1))
